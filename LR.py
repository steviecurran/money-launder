...
print("Validation accuracy = %1.3f" %(acc*100))
...
clf = GridSearchCV(classifier, param_grid = param_grid, cv = cv, verbose=True, n_jobs=-1)
best_clf = clf.fit(X,y) # CROSS VALIDATING (cv) SO OKAY TO USE WHOLE DATASET
